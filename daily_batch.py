...
def run_daily_batch():
    ...
    if os.path.exists("tickers.txt"):
        print("📁 tickers.txt から監視リストを読み込みます...")
    ...
    try:
        ...
        print(f"⚠️ マクロ指標の取得に失敗しました: {e}")
        macro_returns = pd.DataFrame()

    for name, ticker in tickers.items():
        print(f"📊 {name} ({ticker}) の分析中...")
        ...
        try:
            ...
            if data.empty:
                print(f"❌ {ticker} のデータ取得に失敗しました。スキップします。")
                continue
            ...
            for tr_idx, te_idx in tscv.split(X_scaled):
                ...
                m.fit(X_scaled.iloc[tr_idx], historical_data['Target_Class'].iloc[tr_idx])
                accs.append(accuracy_score(historical_data['Target_Class'].iloc[te_idx], m.predict(X_scaled.iloc[te_idx])))
            wf_accuracy = np.mean(accs)
            ...
            for h_key, h_params in tb_horizons.items():
                ...
                if len(valid_idx) > 100:
                    ...
                    clf_h.fit(X_scaled.loc[valid_idx], data.loc[valid_idx, target_col])
                    cls = list(clf_h.classes_)
                    p1 = clf_h.predict_proba(latest_X_scaled)[0][cls.index(1)] if 1 in cls else 0.0
                    tb_probs[h_params['name']] = round(float(p1 * 100), 2)
                else:
                    tb_probs[h_params['name']] = 0.0

            per_str = f"{round(per, 1)}倍" if pd.notna(per) else "-"
            res = {
                "Date": today_str,
                "銘柄名": name,
                "WF平均正解率(明日)": round(float(wf_accuracy * 100), 2),
                "PER(割安)": per_str,
    ...
    if results:
        ...
        print(f"✨ 全 {len(results)} 件のデータを {csv_file} に追記しました。")
